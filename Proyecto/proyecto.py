...
def consultarSaldo(usuario):
    print("\nConsultando Saldo...")
    print("Su saldo actual es de " + str(usuario["Saldo"]))
...
def pagarFacturas(usuario):
    ...
    for factura in usuario["FacturasAdeudadas"]:
        if usuario["Saldo"] >= factura["Valor"]:
            ...
            usuario["FacturasAdeudadas"].remove(factura)
            print("Se ha pagado la factura con exito, tu saldo restante es de " + str(usuario["Saldo"]))
        else:
            print("No cuenta con el saldo suficiente para pagar sus facturas")
...
def consultarSucursales():
    ...
    for sucursal in sucursales:
        print("zona " + sucursal + " - " + str(sucursales[str(sucursal)]))
...
def sacarTurno():
    ...
    for i in range(1, 8):
        diasDisponibles.append(hoy + datetime.timedelta(days=i))

    diaSeleccionado = diasDisponibles[0]
    print("Turno asignado para el día",
      diaSeleccionado.strftime('%d-%m-%Y'))
...
# MENSAJE INICIAL
print("\n**Soy Telmo, tu asistente virtual**")

# SOLICITAR DNI CON input() Y ALMACENARLO EN UNA VARIABLE
dni = int(input("Favor de ingresar su DNI "))
...
print("¡Hola! " + usuarioActual["Nombre"])
...
while continuar == "SI":
    ...
    confirmacion = input("Quizo seleccionar la opcion numero " + opcion + "? ")
    if confirmacion == "NO":
        continue
    ...
    if opcion == "1":
        ...
        print("La opcion" + opcion + "no existe, intente nuevamente")
        continue
    # PREGUNTAR SI DESEA CONTINUAR
    continuar = input("Desea realizar otra operacion mas? ").upper()
