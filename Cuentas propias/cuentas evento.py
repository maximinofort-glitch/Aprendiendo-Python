...
os.chdir(r"C:\Users\maxim\OneDrive\Documentos\Codigos\Cuentas propias")
#os.chdir(r"C:\Users\maxim\Documents\Aprendiendos-Python\Cuentas propias")
...
def abrir():
    ...
    with open("Total.txt", "r") as f:
        datos = [linea.strip() for linea in f.readlines()]
    Diccionarios["total"]["banamex"] = float(datos[0])
    Diccionarios["total"]["bbva"] = float(datos[1])
    Diccionarios["total"]["efectivo"] = float(datos[2])
...
def cerrar(Diccionarios):
    ...
    with open("Total.txt", "w") as t:
        t.write(str(Diccionarios["total"]["banamex"]))
        t.write("\n")
        t.write(str(Diccionarios["total"]["bbva"]))
        t.write("\n")
        t.write(str(Diccionarios["total"]["efectivo"]))
...
def revisar(Diccionarios):
    ...
    total = f"{color('lazul', 'Total')}: Muestra el monto de dinero total, lo que hay en las tarjetas (banamex o bbva) y el efectivo"
    ...
    while lista != "menu":
        ...
        if lista in ("ganancias", "gastos"):
            ...
            print(
                f"      {color('rojo', 'Dinero en banamex' )} : {Diccionarios['total']['banamex']}\n"
                f"      {color('azul', 'Dinero en bbva' )} : {Diccionarios['total']['bbva']}\n"
                f"      {color('verde', 'Dinero en efectivo' )} : {Diccionarios['total']['efectivo']}\n"

                f"      {color('amarillo', 'Dinero total es')} : {(Diccionarios['total']['banamex'] + Diccionarios['total']['bbva'] + Diccionarios['total']['efectivo'])}\n"
                "}"
                )
            print("")
...
def añadir_entrada(accion, lista, Diccionarios):
    ...
    if accion == "deposito":
            ...
            ubicacion = input(f"{color('lmagenta', 'Introduce la ubicacion del dinero (banamex, bbva o efectivo)')}: ")
